...
public class InspectionProcess {
    ...
    public InspectionProcess(String no, int qtt, LocalDateTime date, String productName, String ordersId, String company) {
        this.no = no;
        this.qtt = qtt;
        this.date = date;
    ...
    public void startInspection() {
        ...
        if (no != null && qtt > 0 && date != null && productName != null && ordersId != null && company != null) {
            ...
            // 검사 소요 시간 계산
            int inspectionTime = (qtt / inspectionCnt) * 60; // 제품수에 따른 검사 소요 시간(분)
            int addPtTime = ((inspectionTime / 60) / 1) * ptTime; // 시작 후 1시간마다 준비 시간(분) 추가
            int leadTime = inspectionTime + addPtTime; // 총 검사 소요 시간(분)
